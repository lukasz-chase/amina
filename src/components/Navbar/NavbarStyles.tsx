...
export const Logo = styled.div<NavProps>`
  display: flex;
  align-items: center;
...
export const Buttons = styled.div<NavProps>`
  ...
  .login{
    ...
    border:${({ darkMode }) => (darkMode ? "1px solid white" : "1px solid #1484D6 ")};
    &:hover {
  ...
  .sign-up {
    ...
    border:${({ darkMode }) => (darkMode ? "1px solid #0e0e0f" : "1px solid #1484D6")};
  }
`
...
export const Account = styled.div<NavProps>`
  ...
  height: 35px;
  margin-right: 1rem;
  border:${({ darkMode }) => (darkMode ? "1px solid #1a1a1b" : "1px solid white")} ;
  cursor: pointer;
  &:hover {
